...
def lineIntersect(l1, l2, p1, p2):
    ...
    parallelthreshold = 1e-8
    try:
        if abs((p2[0]-p1[0])*(l2[1]-l1[1]) - (p2[1]-p1[1])*(l2[0]-l1[0])) > parallelthreshold:
        ...
        print("Error in line intersesct: {}, {}, {}, {}".format(l1, l2, p1, p2))

#Get intersects of line l1 to l2 with convex polygon
def getPolyLineIntersects(poly, l1, l2):
    assert not (l1[0] == l2[0] and l1[1] == l2[1]), print("{}, {}".format(l1, l2))
    intersects = []
    distances = []
    if l1[0] == l2[0]:
        for i in range(len(poly)):
            ...
            if (p1[0] < l1[0] and p2[0] > l1[0]) or (p1[0] > l1[0] and p2[0] < l1[0]):
                ...
                distances.append((l2[1]-l1[1])*(pinter[1]-l1[1]))
    else:
...
def getPolyIntersectArea(poly1, poly2):
    p1 = Polygon(poly1)
    p2 = Polygon(poly2)
    intersects = p1.intersection(p2)
    retintersects = []
    if intersects.type == 'Polygon':
        retintersects.append(list(map(lambda x : [x[0], x[1]], list(intersects.exterior.coords)[:-1])))
    else:
        for intersect in intersects:
            if intersect.type == 'Polygon':
                retintersects.append(list(map(lambda x : [x[0], x[1]], list(intersect.exterior.coords)[:-1])))
    return list(map(lambda x : list(reversed(x)), retintersects))
...
def getPolyIntersectArea(poly1, poly2):
    ...
    if len(intersections) > 0:
        intersections = list(filter(lambda x : getArea(x) > 0, intersections)) #TODO: unsure if negative regions do exist/should be included
...
def mergePolys(poly1, poly2):
    ...
    raise Exception("Failure to merge: {}, {}".format(poly1, poly2))

def pointRightOfLine(p, l1, l2):
...
def pointLeftOfLine(p, l1, l2):
    v1 = [p[0]-l2[0], p[1]-l2[1]]
    v2 = [l1[0]-l2[0], l1[1]-l2[1]]
    if v1[0]*v2[1]-v1[1]*v2[0] > 0:
        return True
    return False