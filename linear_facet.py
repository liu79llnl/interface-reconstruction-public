...
def getLinearFacetFromNormal(poly, a, normal, epsilon):
    ...
    normal_magnitude = getDistance(normal, [0, 0])
    normal = [normal[0]/normal_magnitude, normal[1]/normal_magnitude]
    tangent = [normal[1], -normal[0]]

    vertex_up = None
    vertex_up_dot = -float('inf')
    vertex_down = None
    vertex_down_dot = float('inf')

    dtbase = 1e-10

    #Find topmost and bottommost vertices in poly
    for point in poly:
        point_proj = point[0]*normal[0]+point[1]*normal[1]
        if point_proj > vertex_up_dot:
            vertex_up = point
            vertex_up_dot = point_proj
        if point_proj < vertex_down_dot:
            vertex_down = point
            vertex_down_dot = point_proj
